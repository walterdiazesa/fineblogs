...
const handler = async (req, res) => {
    if (!(req.headers && req.headers.authorization)) {
        return res.status(400).json({ error: 'You have to be logged to like a comment' })
    }
    ...
    try {
        ...
        const docRef = getFirebaseAdmin().firestore().collection('blogs').doc(req.body.bloguuid).collection('comments').doc(req.body.commentuuid).collection('likes').doc(userJWT.email)
